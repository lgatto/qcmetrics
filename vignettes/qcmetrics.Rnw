...
The \texttt{DESCRIPTION} file would need to be updated. The packages \Rpackage{qcmetrics}, \Rpackage{affy} and \Rpackage{yaqcaffy} would need to be specifed as dependencies in the \texttt{Imports:} line and imported in the \texttt{NAMESPACE} file. The documentation file \texttt{RnaDegQC/man/rnaqc.Rd} and the (optional) \texttt{RnaDegQC/man/RnaDegQC-packge.Rd} would need to be updated. 
...
\item Maybe elaborate on difference between single sample vs multiple sample QC, and how to distinguish them in the latter case?
\end{itemize}

...
We will use the \Robject{refA} Affymetrix arrays from the \Rpackage{MAQCsubsetAFX} package as an example data and investigate the RNA degradation using the \Rfunction{AffyRNAdeg} from \Rpackage{affy} \citep{Gautier:2004} and the actin and GAPDH $\frac{3'}{5'}$ ratios, as calculated in the \Rpackage{yaqcaffy} package \citep{yaqcaffy}. The first code chunk demonstrate how to load the data and compute the QC data\footnote{%%
  The pre-computed objects can be directly loaded \Rfunction{load(system.file("extdata/deg.rda", package = "qcmetrics"))} and \Rfunction{load(system.file("extdata/deg.rda", package = "qcmetrics"))}.}.
...
% \subsubsection*{The meta-data section}

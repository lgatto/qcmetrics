...
Quality control (QC) is an essential step in any analytically process. Data of poor quality can at best lead to the absence of positive results or, much worse, it false positives that stem from uncaught faulty and noisy data and much wasted resources in pursuing red herrings.  
...
To illustrate a simple QC analysis for proteomics data, we will download data set \texttt{PXD00001} from the ProteomeXchange repository in the \texttt{mzXML} format \citep{Pedrioli:2004}. The MS$^2$ spectra from that mass-spectrometry run are then read into \R and stored as an \Robject{MSnExp} experiment using the \Rfunction{readMSData} function from the \Rpackage{MSnbase} package \citep{Gatto:2012}. 
...
In this section, we describe a set of $^{15}$N metabolic labelling QC metrics. The data is available as an \Robject{MSnSet} instance and was processed with in-house tools. Briefly, MS$^2$ where search with the Mascot search engin and identification scores adjusted with Mascot Percolator. Heavy and light pairs were then searched in the survey scans and incorporation was estimated based on the peptide sequence and the profile of the heavy form (the \Robject{inc} feature variable). Heavy and light peptides isotopic envelope areas were finally integrated to obtain unlabelled and $^{15}$N quantitation data. The \Robject{pms} object provides such data for PSMs (peptide spectrum matches) with a posterior error probability < 0.05 that can be uniquely matched to proteins.
...
The first QC item examines the $^{15}$N incorporation rate, available in the \Robject{inc} feature variable. We also defined a median incorporation rate threshold \Robject{tr} or 97.5 that is used to set the QC status. We then implement a custom \Rfunction{show} method, that print 5 summary values of the variable's distribution. We finally defined the metric's \Rfunction{plot} function that represent the distribution of the PSM's incorporation rates as a boxplot, shows all the individual rates as jittered dots and represents the \Robject{tr} threshold as a dotted red line. 

${15}^$N experiments of good quality are characterised with high incorporation rates, which allow to deconvolute the heavy and light peptide isotopic envelopes and acurate quantification.
...
plot(qcinc) <- function(object) {
    ...
    geom_boxplot(fill = "#FFFFFFD0", colour = "#000000", outlier.size = 0) +
    geom_hline(yintercept = tr, colour = "red", linetype = "dotted",
                 size = 1) +
...
pep <- combineFeatures(psm,
                       groupBy=as.character(fData(psm)$Peptide_Sequence), 
                       "median", verbose = FALSE)
modpep <- combineFeatures(psm,
                          groupBy=modseq,
                          "median", verbose = FALSE)
prot <- combineFeatures(psm,
                        groupBy=as.character(fData(psm)$Protein_Accession), 
                        "median", verbose = FALSE)
@

Each log$_2$ fold-change distributions  
...
The report generation is handled by dedicated packages, in particular the \Rpackage{knitr} \citep{Xie:2013} and \Rpackage{markdown} \cite{markdown} packages. 

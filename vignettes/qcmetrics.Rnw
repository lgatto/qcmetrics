...
Quality control (QC) is an essential step in any analytical process. Data of poor quality can at best least to the absence of positive results or, much worse, it false positives that stem from uncaught faulty and noisy data and much wasted ressources in pursuing red herrings.  

QC is a prime example where standardisation and automation are of great benefit. A great variety of QC metrics, software and pipelines have been described for any assay commonly used in modern biology. This document present a different tool for QC, whose main features are flexibility and versatility. The \Rpackage{qcmetrics} package is a general framework for QC and quality assessment (QA) that can accomodate any type of data. It provide a flexible framework to implement QC items, that store relevant QC metrics and the visualisation mechanism. These individual items can the be bundled into higher level QC containers that can be readily used to generate reports in various formats. As a result, it becomes easy to develope complete custom pipelines from scratch, automate the generation of reports. The pipelines can easily be updated to accomodate new QC items of better visualisation techiques. 

In section \ref{sec:qcclasses}, we provide an overview of the framework. In section \ref{sec:pipline}, we use microarray (subsection \ref{sec:marray}) and proteomics data (subsection \ref{sec:prot}) to demonstrate the elaboration of QC pipelines: how to create infividual QC objects, how to bundle them to create sets of QC metrics and how to generate reports in multiple formats. We also show how the above steps can be fully automated through simple wrapper functions \ref{sec:wrapper}. Although kept simple in the interest of time and space, these examples are meaningful and relevant. In section \ref{sec:report}, we provide more detail about the report generation process, how reports can be customised and how new exports can be contributed. We proceed in section \ref{sec:qcpkg} on the consolidation of QC pipelines using \R  and elaborate on the development of dedicate QC packages with \Rpackage{qcmetrics}. 
...
\begin{itemize}
\item May be also a high-level example with mzTab?
\end{itemize}
...
The generation of the \texttt{html} report is enabled by the creation of a \R markdown file (\texttt{Rmd}) that is them converted with \Rpackage{knitr} and \Rpackage{markdown} into \texttt{html}. The \texttt{Rmd} synatex being much simpler, no \texttt{Rmd} template is needed. It is however possible to customise the final \texttt{html} output by providing a \texttt{css} definition as \Robject{template} argument\marginnote{\textbf{TODO}}.  when calling \Rfunction{qcReport}.
